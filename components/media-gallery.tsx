...
export function MediaGallery() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#fbddd9] px-4 pb-24 pt-14 text-[#4f0b47] sm:px-8">
      <svg
        className="pointer-events-none absolute -left-16 -top-36 z-0 h-96 w-[520px] text-[#a77278]/28"
        viewBox="0 0 520 380"
        fill="none"
        aria-hidden="true"
      >
        ...
        <div className="mb-10 flex flex-col items-start gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="group inline-flex h-14 items-center gap-2.5 rounded-full bg-[#4f0b47] pl-7 pr-6 text-[14px] font-semibold uppercase tracking-[0.12em] text-[#ffe8e4] shadow-[0_14px_34px_rgba(79, 11, 71,0.22)] transition-transform duration-300 hover:scale-[1.04]"
            >
              {brand.media.ctaLabel}
              <span className="grid h-7 w-7 place-items-center rounded-full bg-[#ffe8e4]/18 transition-transform duration-300 group-hover:translate-x-0.5">
          ...
          {items.map((image) => {
            ...
            return (
              <div
                key={image.src}
                data-media-card
                className="relative aspect-[3/4] w-[80%] shrink-0 snap-start overflow-hidden rounded-[26px] bg-[#f8c5be] shadow-[0_20px_54px_rgba(79, 11, 71,0.16)] sm:w-[46%] lg:w-[calc((100%-2.5rem)/3)]"
              >
                {isVideo ? (
                  <video
                    src={image.src}
...
function CarouselButton({
  ...
  return (
    <button
      ...
      className="grid h-14 w-14 place-items-center rounded-full border border-[#4f0b47]/18 bg-[#ffe8e4] text-[#4f0b47] transition-all duration-300 hover:border-[#4f0b47] hover:bg-[#4f0b47] hover:text-[#ffe8e4] disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-[#4f0b47]/18 disabled:hover:bg-[#ffe8e4] disabled:hover:text-[#4f0b47]"
    >
