    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.4fr]">
      ...
      <div className="spabox-card flex flex-col p-5 sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[#4f0b47]/14 bg-[#ffe8e4] px-4 text-sm font-semibold text-[#4f0b47] transition-colors hover:border-[#4f0b47]/40"
          ...
          >
            ...
            Filters
            {activeFilterCount > 0 && (
              <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#4f0b47] px-1.5 text-[11px] font-bold text-[#ffe8e4]">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        <label className="relative mb-3 block">
          ...
          <SearchIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#b35b54]" />
          <input
            type="search"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder={searchPlaceholder}
            className="h-11 w-full rounded-full border border-[#4f0b47]/14 bg-[#ffe8e4] pl-12 pr-4 text-sm font-medium text-[#4f0b47] outline-none transition-colors placeholder:text-[#b35b54] focus:border-[#4f0b47]/40"
          />
        </label>
        ...
        <p className="mb-2 text-xs font-medium text-[#5c2252]/68">
        ...
        <div className="no-scrollbar max-h-[420px] space-y-2 overflow-y-auto lg:max-h-[560px]">
          {filtered.length === 0 ? (
            <div className="rounded-[18px] border border-dashed border-[#4f0b47]/20 p-6 text-center">
              <p className="m-0 text-sm font-medium text-[#5c2252]/72">No treatments match.</p>
              <button
                type="button"
                onClick={() => {
                  clearAll();
                ...
                className="mt-2 text-sm font-semibold text-[#b35b54] underline-offset-4 hover:underline"
              >
                Reset
              </button>
            </div>
                ...
                <button
                  ...
                  className={[
                    ...
                    active
                      ...
                      : "border-[#4f0b47]/12 bg-[#ffe8e4]/68 text-[#4f0b47] hover:border-[#4f0b47]/35",
                  ].join(" ")}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="m-0 truncate text-sm font-semibold">{treatment.name}</p>
                      <p className={`m-0 mt-0.5 text-xs ${active ? "text-[#ffe8e4]/72" : "text-[#5c2252]/68"}`}>
                        {treatment.duration_minutes ? `${treatment.duration_minutes} min · ` : ""}
                        {brand.currency} {treatment.default_price}
                      </p>
                    </div>
                    {active && (
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#ffe8e4] text-xs text-[#4f0b47]">
                        ✓
                      </span>
                    )}
                    {treatment.image_url && (
      ...
      <div className="spabox-card p-4 sm:p-6">
        ...
        <button
          ...
          className="mt-6 inline-flex w-full items-center justify-center gap-2 bg-[#4f0b47] px-5 py-3 text-sm font-semibold text-[#ffe8e4] transition-colors hover:bg-[#3a0c34] disabled:cursor-not-allowed disabled:opacity-50"
        >
          {submitting
            ? "Confirming…"
            : selectedSlot
